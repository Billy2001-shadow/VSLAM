...
void LoopClosing::ProcessNewKeyframe() {
  ...
  orb_extractor_->ScreenAndComputeKPsParams(
      current_keyframe_->left_img_, pyramid_points,
      current_keyframe_->pyramid_key_points_);

  // calculate the orb descriptors of all valid pyramid keypoints
  orb_extractor_->CalcDescriptors(current_keyframe_->left_img_,
                                  current_keyframe_->pyramid_key_points_,
...
std::vector<cv::Mat>
LoopClosing::ConvertToDescriptorVector(const cv::Mat &descriptors) {
...
bool LoopClosing::DetectLoop() {
  ...
  for (const auto &db : key_frame_database_) {
    if (current_keyframe_->key_frame_id_ - db.second->key_frame_id_ < 20)
      break;
    if (!current_keyframe_->bow2_vec_.empty() &&
        !db.second->bow2_vec_.empty()) {
      float similarity_score = dbow2_vocabulary_->score(
          current_keyframe_->bow2_vec_, db.second->bow2_vec_);
  ...
  LOG(INFO) << "----------------------------------------------------" << TAIL;
  LOG(INFO) << GREEN << "LoopClosing: find potential Candidate KF. Score is "
            << max_score
...
bool LoopClosing::MatchFeatures() {
  ...
  cv_matcher_->match(loop_keyframe_->ORBDescriptors_,
                     current_keyframe_->ORBDescriptors_, matches);
  //计算最大距离和最小距离
  auto min_max =
  ...
  for (const auto &mt : matches) {
    //当描述子之间的距离大于两倍的最小距离时，即认为匹配有误，但有时候最小距离会非常小，所以要设置一个经验值30作为下限。
    if (mt.distance <= std::max(2 * min_distance, 30.0)) {
      /// class_id indicates the number of feature points, the pyramid is
      /// removed, and the class_id of the same pyramid level is the same
      int loop_feature_id =
          loop_keyframe_->pyramid_key_points_[mt.queryIdx].class_id;
      int current_feature_id =
          current_keyframe_->pyramid_key_points_[mt.trainIdx].class_id;

      /// the matches of keypoints belonging to the same feature pair shouldn't
      /// be inserted into the valid matches twice
      if (set_valid_feature_matches_.find(
              {current_feature_id, loop_feature_id}) !=
...
bool LoopClosing::ComputeCorrectPose() {
  ...
  for (auto iter = set_valid_feature_matches_.begin();
    ...
    MapPoint::Ptr mp =
        loop_keyframe_->features_left_[loop_feature_id]->map_point_.lock();
    if (mp) {
      Eigen::Vector3d p = mp->Pos();
      current_point2f.push_back(
          current_keyframe_->features_left_[current_feature_id]->position_.pt);
  ...
  if (show_loop_closing_result_) {
    ...
    cv::drawMatches(current_keyframe_->left_img_,
                    current_keyframe_->GetKeyPoints(),
                    loop_keyframe_->left_img_, loop_keyframe_->GetKeyPoints(),
                    matches_with_mappoint, img_goodmatch);
  ...
  // use "try - catch" since cv::solvePnPRansac may fail because of terrible
  // match result and I don't know why the result of solvePnPRansac() is
  // sometimes not reliable
  //      even the reprojection error of inlier is high
  try {
    cv::solvePnPRansac(loop_point3f, current_point2f, K, cv::Mat(), rvec, tvec,
                       false, 100, 5.991, 0.99);
  } catch (...) {
    return false;
  }
  ...
  if (cnt_inliner < 10)
    return false;
  LOG(INFO) << GREEN
            << "LoopClosing: number of match inliers (after optimization): "
            << cnt_inliner << TAIL;

  double error =
      (current_keyframe_->getPose() * corrected_current_pose_.inverse())
          .log()
          .norm();
  if (error > 1 && error < 15) {
...
int LoopClosing::OptimizeCurrentPose() {
  ...
  typedef g2o::LinearSolverDense<BlockerSolverType::PoseMatrixType>
      LinerrSolverType;
  auto solver = new g2o::OptimizationAlgorithmLevenberg(
      g2o::make_unique<BlockerSolverType>(
          g2o::make_unique<LinerrSolverType>()));
  ...
  int index = 1;
  Eigen::Matrix3d left_cam_K = left_camera_->K();
  Eigen::Matrix3d right_cam_K = right_camera_->K();
  ...
  for (auto &match : set_valid_feature_matches_) {
    ...
    Eigen::Vector2d observe = to_vec2(feat->position_.pt);
    Eigen::Vector3d pt3d = loop_keyframe_->features_left_[loop_feature_id]
                               ->map_point_.lock()
                               ->Pos();
...
void LoopClosing::LoopCorrect() {
  ...
  static unsigned long cnt_loopclosing = 1;
  cout << "第" << cnt_loopclosing << "次闭环优化" << endl;
  for (int i = 0; i < 100000; i++) {
    std::cout << cnt_loopclosing;
  }
  cnt_loopclosing++;
...
void LoopClosing::CorrectActivateKeyframeAndMappoint() {
  ...
  for (auto &kf : map_->GetActiveKeyFrames()) {
    ...
    /// Get the relative pose between two keyframes, the relative pose is
    /// constant
    Sophus::SE3d T_kn_k =
        kf.second->getPose() * current_keyframe_->getPose().inverse();
    /// T_a = T_{{k-n}_k} * T_{kt_w}
    /// Calculate the correct pose of the key frame based on the correct pose of
    /// the current frame
  ...
  for (auto &mappoint : map_->GetActiveMapPoints()) {
    ...
    /// correct the mappoint's position according to the corrected pose of
    /// active KF which first observes it and the relative position between them
    ...
    if (correct_activate_pose.find(observe_kf->key_frame_id_) ==
      ...
      continue;
    /// P_cam = T_cw * P_w
    Eigen::Vector3d pos_cam = observe_kf->getPose() * mp->Pos();
    Sophus::SE3d correct_pose =
        correct_activate_pose.at(observe_kf->key_frame_id_);
    mp->SetPos(correct_pose.inverse() * pos_cam);
  }
  ...
  for (auto iter = set_valid_feature_matches_.begin();
       iter != set_valid_feature_matches_.end(); iter++) {
    int current_feature_id = iter->first;
    int loop_feature_id = iter->second;
    auto loop_mp =
        loop_keyframe_->features_left_[loop_feature_id]->map_point_.lock();
    // assert(loop_mp != nullptr);
    auto current_mp = current_keyframe_->features_left_[current_feature_id]
                          ->map_point_.lock();
    ...
    if (current_mp && loop_mp) {
      ...
      current_keyframe_->features_left_[current_feature_id]->map_point_ =
          loop_mp;
    }
  }
...
void LoopClosing::PoseGraphOptimization() {
  ...
  { // mutex
    ...
    // set the mappoints' positions according to its first observing KF's
    // optimized pose
    auto allMapPoints = map_->GetAllMapPoints();
    auto activeMapPoints = map_->GetActiveMapPoints();
    for (auto iter = activeMapPoints.begin(); iter != activeMapPoints.end();
         iter++) {
    ...
    for (auto &mappoint : allMapPoints) {
      ...
      if (vertices_kf.find(observingKF->key_frame_id_) == vertices_kf.end()) {
        // NOTICE: this is for the case that one mappoint is inserted into map
        // in frontend thread but the KF which first observes it hasn't been
        // inserted into map in backend thread
        continue;
      }
      Eigen::Vector3d posCamera = observingKF->getPose() * mp->Pos();

      Sophus::SE3d T_optimized =
          vertices_kf.at(observingKF->key_frame_id_)->estimate();
      mp->SetPos(T_optimized.inverse() * posCamera);
    }

    unsigned long frontend_ref_kf_id =
        frontend_->getReferenceKF()->key_frame_id_;
    ...
    for (auto &v : vertices_kf) {
      if (v.first == frontend_ref_kf_id) {
        ...
        //   Sophus::SE3d ref_kf_old_pose =
        //   frontend_->getReferenceKF()->getPose(); Sophus::SE3d T_k_old_new =
        //   ref_kf_old_pose.inverse() * ref_kf_new_pose;

        //   Sophus::SE3d new_current_frame_rel_pose =
        //   front_curr_frame->getRelativePose() * T_k_old_new;
        //   front_curr_frame->SetRelativePose(new_current_frame_rel_pose);
        //   Sophus::SE3d new_last_frame_rel_pose =
        //   front_last_frame->getRelativePose() * T_k_old_new;
        //   front_last_frame->SetRelativePose(new_last_frame_rel_pose);
        //   LOG(INFO) << YELLOW << "Find FRAME, Pose error: " <<
        //   T_k_old_new.log().norm() << TAIL;
        continue;
      }
...
void LoopClosing::AddToKeyframeDatabase() {
  key_frame_database_.insert(
      {current_keyframe_->key_frame_id_, current_keyframe_});
}
