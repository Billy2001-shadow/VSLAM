...
void Map::InsertKeyFrame(KeyFrame::Ptr keyframe) {
  //这个肯定得改成KeyFrame
  current_frame_ = keyframe;
  if (keyframes_.find(keyframe->keyframe_id_) == keyframes_.end()) {
    keyframes_.insert(make_pair(keyframe->keyframe_id_, keyframe));
    active_keyframes_.insert(make_pair(keyframe->keyframe_id_, keyframe));
  } else {
    keyframes_[keyframe->keyframe_id_] = keyframe;
...
void Map::RemoveMapPoint(std::shared_ptr<MapPoint> mappoint) {
  ...
  landmarks_.erase(mpId);

  // delete from active mappoints
  active_landmarks_.erase(mpId);
}
