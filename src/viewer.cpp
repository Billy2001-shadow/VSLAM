...
void Viewer::UpdateMap() {
  ...
  std::cout << "现在地图中共有 " << active_landmarks_.size() << " ge点 "
            << std::endl;
  map_updated_ = true;
}
...
void Viewer::DrawMapPoints() {
  //后面active_landmarks_没有再更新了
  std::cout << "地图点数量::::::::::::::::::::" << active_landmarks_.size()
            << std::endl;
  const float red[3] = {1.0, 0, 0};
  for (auto &kf : active_keyframes_) {
    DrawFrame(kf.second, red); //画关键帧
  }

  glPointSize(2);
  glBegin(GL_POINTS);
  for (auto &landmark : active_landmarks_) {
    auto pos = landmark.second->Pos(); //画地图点
