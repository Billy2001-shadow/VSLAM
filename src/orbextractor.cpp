...
static bool isFastCorner(cv::Mat &img, cv::KeyPoint &keypoint, int threshold) {
  ...
  const uchar *ptr =
      img.ptr<uchar>(cvRound(keypoint.pt.y)) + cvRound(keypoint.pt.x);
...
void ORBextractor::Detect(InputArray _image, InputArray _mask,
  ...
  for (int i = 0; i < nRows; i++) {
    ...
    for (int j = 0; j < nCols; j++) {
      ...
      vector<cv::KeyPoint> vKeysCell;
      FAST(image.rowRange(iniY, maxY).colRange(iniX, maxX), vKeysCell,
           iniThFAST, true);
...
void ORBextractor::ScreenAndComputeKPsParams(
    InputArray _image, vector<KeyPoint> &_keypoints,
  ...
  for (size_t i = 0, N = _keypoints.size(); i < N; i++) {
    ...
    if (!(kps.pt.y - EDGE_THRESHOLD >= 0 &&
          kps.pt.y + EDGE_THRESHOLD < imgPyramid.rows &&
          kps.pt.x - EDGE_THRESHOLD >= 0 &&
          kps.pt.x + EDGE_THRESHOLD < imgPyramid.cols)) {
      kps.pt *= scale;
      continue;
    }
...
void ORBextractor::CalcDescriptors(InputArray _image,
  ...
  for (int level = 0; level < nlevels; level++) {
    ...
    cv::GaussianBlur(workingMat, workingMat, Size(7, 7), 2, 2,
                     BORDER_REFLECT_101);
  ...
  for (int level = 0; level < nlevels; ++level) {
    ...
    if (level != 0) {
      resize(mvImagePyramid[level - 1], mvImagePyramid[level], sz, 0, 0,
             INTER_LINEAR);
      resize(mvMaskPyramid[level - 1], mvMaskPyramid[level], sz, 0, 0,
             INTER_LINEAR);
    }
  }
}
